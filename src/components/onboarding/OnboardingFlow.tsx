...
export default function OnboardingFlow() {
  ...
  const completeOnboarding = () => {
    // Save all onboarding data + diagnostic results to localStorage
    let p = loadProgress();

    // Record diagnostic as a session
    if (answers.length > 0) {
      p = recordSession(p, answers, elapsed, data.focus);
      const ok = answers.filter((a) => a.correct).length;
      p.diagnosticScore = Math.round((ok / answers.length) * 100);
    }
  ...
  if (step === 7) return <ScheduleSetup key={ak} data={data} setData={setData} studyDays={studyDays} setStudyDays={setStudyDays} onComplete={completeOnboarding} ProgressBar={ProgressBar} />;
  return null;
}
