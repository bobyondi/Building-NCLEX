...
export default function Nav() {
  ...
  useEffect(() => {
    ...
    setHasOnboarded(p.onboarding.completed);
  }, []);
    ...
    <nav className="sticky top-0 z-50" style={{ background: "rgba(251,247,243,0.92)", backdropFilter: "blur(12px)", borderBottom: "1px solid hsl(var(--border))" }}>
      <div className="max-w-[1100px] mx-auto px-6 md:px-6 flex justify-between items-center h-14">
        ...
        <div className="hidden md:flex items-center gap-5">
          ...
          {hasOnboarded && (
            <button className="btn btn-g" onClick={() => navigate("/dashboard")} style={{ fontSize: 13, gap: 5 }}>
              <LayoutDashboard size={14} /> Dashboard
            </button>
          )}
          <button className="btn btn-p btn-sm" onClick={() => navigate(hasOnboarded ? "/dashboard" : "/onboarding")}>
            {hasOnboarded ? "Continue Studying" : "Get a Study Plan"}
          </button>
        </div>
      </div>
    </nav>
